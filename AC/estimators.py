...
class PolicyEstimator():
    ...
    def predict(self, states, sess=None):
        ...
        return sess.run(self.predictions['probs'], { self.states: states })

    def update(self, states, targets, actions, sess=None):
        sess = sess or tf.get_default_session()
        feed_dict = { self.states: states, self.targets: targets, self.actions: np.array([actions,])  }
...
class ValueEstimator():
    ...
    def predict(self, states, sess=None):
        sess = sess or tf.get_default_session()
        return sess.run(self.predictions['logits'], { self.states: states })

    def update(self, states, targets, sess=None):
        sess = sess or tf.get_default_session()
        feed_dict = { self.states: states, self.targets: targets }
        _, loss = sess.run([self.train_op, self.loss], feed_dict)
        return loss
